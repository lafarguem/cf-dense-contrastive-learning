...
@torch.no_grad()
def plot_counterfactual_viz_mimic(args, x, cf_loc, pa, cf_pa, do, rec_loc, cf_x, name, save=True):
    fs = 15
    m, s = 7, 4
    n = 4
    ...
    for j in range(n):
        ...
        ax[6, j].set_title("cf_x")

    if save:
        fig.savefig(
            os.path.join(args.save_dir, 'visualizations', f"viz-{args.iter}-{name}.png"), bbox_inches="tight"
        )
        return
...
def plot_counterfactual_viz_embed(args, x, cf_x, pa, cf_pa, do, rec_loc, save=True):
    ...
    if save:
        fig.savefig(
            os.path.join(args.save_dir, 'visualizations', f"viz-{args.iter}.png"), bbox_inches="tight"
        )
        return
    else:
        return fig
...
def write_images(args, model, batch):
    ...
    if "padchest" in args.hps:
        ...
        cf_disease = copy.deepcopy(cf_pa)
        cf_pa["scanner"] = 1 - cf_pa["scanner"]
        cf_disease["disease"] = 1 - cf_disease["disease"]
        do = {"scanner": cf_pa["scanner"]}
        do_disease = {"disease": cf_disease["disease"]}
        _cf_pa = torch.cat([cf_pa[k] for k in args.parents_x], dim=1)
        _cf_disease = torch.cat([cf_disease[k] for k in args.parents_x], dim=1)
        _cf_pa = (
            _cf_pa[..., None, None]
            .repeat(1, 1, *(args.input_res,) * 2)
            .to(args.device)
        ...
        _cf_disease = (
            _cf_disease[..., None, None]
            .repeat(1, 1, *(args.input_res,) * 2)
            .to(args.device)
            .float()
        )
        cf_loc, cf_scale = model.forward_latents(zs, parents=_cf_pa)
        cf_disease_loc, cf_disease_scale = model.forward_latents(zs, parents=_cf_disease)
        ...
        cf_disease_scale = cf_disease_scale * u_t
        cf_disease_x = torch.clamp(cf_disease_loc + cf_disease_scale * u, min=-1, max=1)

        fig1 = plot_counterfactual_viz_mimic(
            args, batch["x"], cf_loc, pa, cf_pa, do, rec_loc, cf_x, 'scanner'
        )
        fig2 = plot_counterfactual_viz_mimic(
            args, batch["x"], cf_disease_loc, pa, cf_disease, do_disease, rec_loc, cf_disease_x, 'disease'
        )
        return fig1, fig2
...
def run_classifier(args, model, classifier, val_loader):
    from counterfactuals.trainer import preprocess_batch

    all_preds = {pa: [] for pa in args.parents_x}
    all_targets = {pa: [] for pa in args.parents_x}

    for sample in val_loader:
        sample = preprocess_batch(args, sample, expand_pa=args.expand_pa)
        zs = model.abduct(x=sample["x"], parents=sample["pa"])
        if model.cond_prior:
            zs = [zs[j]["z"] for j in range(len(zs))]
        pa = {k: sample[k] for k in args.parents_x}
        _pa = torch.cat([sample[k] for k in args.parents_x], dim=1)
        _pa = (
            _pa[..., None, None]
            .repeat(1, 1, *(args.input_res,) * 2)
            .to(args.device)
        ...
        # counterfactuals (focus on changing scanner)
        for i, parent in enumerate(args.parents_x):
            cf_pa = copy.deepcopy(pa)
            cf_pa = {k: sample[k] for k in args.parents_x}
            cf_pa[parent] = 1 - cf_pa[parent]
            do = {parent: cf_pa[parent]}
            _cf_pa = torch.cat([cf_pa[k] for k in args.parents_x], dim=1)
            _cf_pa = (
                _cf_pa[..., None, None]
                .repeat(1, 1, *(args.input_res,) * 2)
            ...
            u = (sample["x"] - rec_loc) / rec_scale.clamp(min=1e-12)
            u_t = 0.1 if "mnist" in args.hps else 1.0  # cf sampling temp
            cf_scale = cf_scale * u_t
            cf_x = torch.clamp(cf_loc + cf_scale * u, min=-1, max=1)        
            ...
            preds = torch.sigmoid(out[:,i]) > 0.5
            all_preds[parent].append(preds.int().cpu())
            all_targets[parent].append(cf_pa[parent].int().cpu())


    classifier_acc = {}
    classifier_f1 = {}

    for parent in args.parents_x:
        all_preds[parent] = torch.cat(all_preds[parent]).numpy()
        all_targets[parent] = torch.cat(all_targets[parent]).numpy()

        f1 = f1_score(all_targets[parent], all_preds[parent], average='macro')
        acc = accuracy_score(all_targets[parent], all_preds[parent])
        classifier_acc[parent] = acc
        classifier_f1[parent] = f1
    return classifier_acc, classifier_f1