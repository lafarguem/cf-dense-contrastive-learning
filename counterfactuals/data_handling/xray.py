...
from counterfactuals.data_handling.base import BaseDataModuleClass
from datetime import datetime
import os
from counterfactuals.data_handling.caching import SharedCache
from counterfactuals.data_handling.augmentations import DataAugmentationDINO
...
def prepare_padchest_csv():
    ...
    print('nf', len(df[df['no finding'] == True]))
    print('pe', len(df[df['pleural effusion'] == True]))
    print('pe only', len(df[df.disease == True]))
    df.disease = df['pleural effusion']

    # df = df[(df.disease == True) | (df['no finding'] == True)]

    print("total len", len(df))
    print("inter len", df[(df.disease == True) & df['no finding'] == True])

    return df

def split_padchest(df, target_test, balance_attibute):
    all_patients = df["PatientID"].unique()

    df_selected = df[df[balance_attibute] == 1].head(target_test // 2)
    df_unselected = df[df[balance_attibute] == 0].head(target_test // 2)

    # Step 2: Combine and deduplicate PatientIDs efficiently
    test_patients_all = pd.unique(
        pd.concat([df_selected["PatientID"], df_unselected["PatientID"]])
    )

    complement_patients = np.setdiff1d(all_patients, test_patients_all)

    df_test = df[df.PatientID.isin(test_patients_all)]

    print("negative", len(df_test[df_test[balance_attibute] == 0]))
    print("positive", len(df_test[df_test[balance_attibute] == 1]))

    return complement_patients, test_patients_all
...
class PadChestDataModule(BaseDataModuleClass):
    def create_datasets(self):
        ...
        train_id, val_id = split_padchest(df, 100, 'disease')
...
class PadChestDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if isinstance(self.transform, DataAugmentationDINO):
            ...
            sample["x"] = img.float()
        # if sample["disease"] == 1:
        #     save_image(sample["x"], 'outputs/test_cf/disease.png')
