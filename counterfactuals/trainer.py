...
from counterfactuals.plotting_utils import write_images, run_classifier
from counterfactuals.utils import linear_warmup
...
def preprocess_batch(args, batch, expand_pa=False):
    ...
    for k in batch.keys():
        if (
            ...
            and not (isinstance(batch[k], List) or not isinstance(batch[k], torch.Tensor))
            and batch[k].ndim == 1
        ):
...
def trainer(args, model, ema, dataloaders, optimizer, scheduler, writer, logger, eval_classifier):
    ...
    iter_val = iter(dataloaders["valid"])
    temp_batch = next(iter_val)
    len_batch = temp_batch["x"].shape[0]
    viz_batch = copy.deepcopy(temp_batch)
    completed = []
    while len(completed) < 4:
        for i in range(len_batch):
            if (0 not in completed or 1 not in completed) and temp_batch["disease"][i].item() == 0:
                if 0 not in completed:
                    for key, val in temp_batch.items():
                        viz_batch[key][0] = val[i]
                    completed.append(0)
                elif 1 not in completed:
                    for key, val in temp_batch.items():
                        viz_batch[key][1] = val[i]
                    completed.append(1)
            if (2 not in completed or 3 not in completed) and temp_batch["disease"][i].item() == 1:
                if 2 not in completed:
                    for key, val in temp_batch.items():
                        viz_batch[key][2] = val[i]
                    completed.append(2)
                elif 3 not in completed:
                    for key, val in temp_batch.items():
                        viz_batch[key][3] = val[i]
                    completed.append(3)
        temp_batch = next(iter_val)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for pa in args.parents_x:
            writer.add_scalar(f"classifier/acc/{pa}", acc[pa], args.epoch)
            writer.add_scalar(f"classifier/f1/{pa}", f1[pa], args.epoch)
            logger.info(
                f'=> classifier {pa} | acc: {acc[pa]:.4f}'
                + f' - f1: {f1[pa]:.4f}'
            )
        save_dict = {
            "epoch": args.epoch,
        ...
        if (args.epoch - 1) % args.eval_freq == 0:
            ...
            if valid_stats["elbo"] < args.best_loss:
                ...
                ckpt_path = os.path.join(args.save_dir, 'checkpoints', "checkpoint.pt")
                torch.save(save_dict, ckpt_path)
                logger.info(f"Model saved: {ckpt_path}")

        ckpt_path = os.path.join(args.save_dir, 'checkpoints', "last.pt")
        torch.save(save_dict, ckpt_path)
        logger.info(f"Model saved: {ckpt_path}")
