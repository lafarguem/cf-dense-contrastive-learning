...
class FilterByPathology(Modifier):
    """
    Filter the labels based on the target pathology
    """

    def __init__(self, pathologies: list[Pathology]):
        self.pathologies = [pathology.value for pathology in pathologies]

    def apply(self, labels: pd.DataFrame) -> pd.DataFrame:
        all_pathologies = [p.value for p in Pathology]
        non_target_cols = [col for col in all_pathologies if col not in self.pathologies]
        
        mask = (labels[non_target_cols] != 1).all(axis=1)

        return labels[mask]

    def __str__(self):
        return f"FilterByPathology({self.pathologies})"